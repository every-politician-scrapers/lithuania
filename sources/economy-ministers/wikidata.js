...
module.exports = function () {
  let startfilter = meta.start ? ` FILTER (!BOUND(?startV) || (?startV >= "${meta.start}T00:00:00Z"^^xsd:dateTime))` : '';
  let endfilter   = meta.end   ? ` FILTER (!BOUND(?endV)   || (?endV   >= "${meta.end}T00:00:00Z"^^xsd:dateTime))`   : '';
